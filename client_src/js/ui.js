...
module.exports.displayTextBox = (scene, text, size, align, pos, rot) => new Promise((resolve, reject) => {
    ...
    loader.load( '/assets/fonts/helvetiker_bold.typeface.json', function ( font ) {
        ...
        const bgGeometry = new THREE.PlaneGeometry(geometry.boundingBox.max.x * 1.1, geometry.boundingBox.max.y * 3.5);
        const bgMaterial = new THREE.MeshBasicMaterial({ color: 0x000000 });
        const bgMesh = new THREE.Mesh(bgGeometry, bgMaterial);

        bgMesh.position.set(posX + geometry.boundingBox.max.x / 2, pos.y - geometry.boundingBox.max.y / 2, pos.z + 0.01);
